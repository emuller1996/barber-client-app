      ...
      <div className="container">
        <div className="row">
          <div className="col-md-4">
            <label htmlFor="date_search">Fecha </label>
            <input
              type="date"
              value={dateSearch}
              name="date_search"
              ...
              id="date_search"
              className="form-control text-center input-appointment"
            />
          </div>
          <div className="col-md-6">
            <label htmlFor="barber_selected">Barbero</label>
            <select
              name="barber_selected"
              id="barber_selected"
              onChange={(e) => {
                setBarberSearch(e.target.value);
              }}
              className="form-control custom-select input-appointment rounded-0"
            >
              <option value="all">Todos</option>
              {barbers &&
                barbers.map((b) => <option value={b._id}>{b.name} </option>)}
            </select>
          </div>
        </div>
        ...
        <div className=" mt-4">
          <div className="row">
            {appointments && appointments.length === 0 && (
              <div className="col-12"> No hay Citas Agendadas. </div>
            )}
            {appointments ? (
              appointments.map((a) => (
                <CardAppointment
